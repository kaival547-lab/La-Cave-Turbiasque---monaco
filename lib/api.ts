...
const checkBackend = async () => {
    try {
        const res = await fetch(`${API_URL}/menu`, { method: 'HEAD' });
...
export const menuAPI = {
    ...
    async getAll() {
        try {
            ...
            if (!isRunning) {
                console.error(`🔴 BACKEND NOT REACHABLE at ${API_URL}. Please ensure "npm run dev" is running and you have restarted your terminal after the fix.`);
            } else {
    ...
    async getPopular() {
        try {
            ...
            const isRunning = await checkBackend();
            if (!isRunning) {
                console.error(`🔴 BACKEND NOT REACHABLE at ${API_URL}. Please ensure the backend server is running.`);
            } else {
